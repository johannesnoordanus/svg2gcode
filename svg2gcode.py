...
# Notes on svg2gcode.py:
#
# - svg2gcode.py must be in $PATH
# - it's classes directory must (currently) be included in the current directory of the executable
# - example command to create two types of gcode file, one containing the drawings of the .svg,
#   the other containing the images .svg with correct transformations (and positioning):
#       > svg2gcode.py ambachtmanlogo.svg logo.gc
#   (the image file is derived from the .gc output name - in this case logo_images.gc -, and contains gcode for all converted images)
# - Note that drawing objects (when using Inkscape for example) must be converted to a 'path' to be translated in a gcode sequence
# - Note also that images objects should not be converted a 'path'.
...
def svg2gcode(args) -> int:
    ...
    try:
        ...
        gcode_compiler = Compiler(interfaces.Gcode, params={'laser_power':args.cuttingpower,'movement_speed':args.cuttingspeed,
                            ...
                            'image_movement_speed':args.imagespeed,'fan':args.fan,'rapid_move':True,'showimage':args.showimage,
